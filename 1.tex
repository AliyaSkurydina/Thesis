...
\end{definition}
\begin{definition}
	Оператор $A$ называется дифференцируемым по Фреше в точке $u\in H$, если в некоторой окрестности этой точки для всех $h\in H$
	$$A(u+h)=A(u)+A'(u)h+\omega(u,h),$$
	где $\frac{\|\omega(u,h)\|}{\|h\|}\to 0$, $\|h\|\to 0$.
	Линейный непрерывный оператор $A'(u):H \to H$ называется производной Фреше оператора $A$ в точке $u$ .
\end{definition}
\begin{definition}
...
\begin{definition}
	...
	\forall u\in H\quad\forall z\in Fix(T) \quad
	{\|u^{k+1}-z\|}^2\le{\|u^k-z\|}^2-\nu{\|u^k-u^{k+1}\|}^2.
	\end{equation}
%	где $\forall u\in H$ $\forall z\in Fix(T)$.
\end{definition}
\begin{definition}
	Пусть $\{x^k\}$ --- сходящаяся последовательность приближений итерационного метода с начальным приближением $x^0\in H$ к $z$ --- решению уравнения. 
	Итерационный метод обладает линейной скоростью сходимости, если $$\exists q\in (0,1)\quad\exists k_0\in\mathbb{N} \quad \forall k\ge k_0: \|x^k-z\|<q\|x^{k-1}-z\|,\quad x^k, z\in H.$$ 
\end{definition}
