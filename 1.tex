...
Выбор начального приближения обусловлен фактом, установленным в статье \cite{Vasin_2016}, где для модифицированных вариантов методов $\eqref{equ_alphaproc}$ доказывается глобальная сходимость итерационных процессов. Параметр $\gamma$ для методов может быть разным, так как не установлен факт сходимости при $\gamma=1$, однако при соответствующем $\gamma<1$ методы $\eqref{equ_rmn}$, $\eqref{equ_alphaproc}$ должны сходиться, этому не противоречат результаты расчетов.
