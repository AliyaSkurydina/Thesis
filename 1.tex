...
\begin{definition}
	...
	Итерационный метод обладает линейной скоростью сходимости, если $$\exists q\in (0,1), \exists k_0\in\mathbb{N}: \|x^k-z\|<q^k\|z-x^0\| \quad \forall k\ge k_0.$$ 
\end{definition}
...
где $\|f-f_\delta\|\le\delta$, $u^0$ --- начальное приближение к решению; а на втором этапе для аппроксимации регуляризованного решения $u_\alpha$ уравнения $\eqref{equ2}$ применяется либо метод Ньютона c параметрами регуляризации $\bar\alpha$, $\alpha$ (РМН)
...
u^{k+1} =u^k - \gamma\frac{\langle [A'(u^k)+\bar{\alpha}I]S_\alpha(u^k), S_\alpha (u^k)\rangle}{\|[A'(u^k)+\bar{\alpha}I]S_\alpha(u^k)\|^2}S_\alpha (u^k).
\end{equation}
...
Так как оператор $A$ --- монотонный, то его производная $A'(u)$ --- неотрицательно определенный оператор. Следовательно, операторы $(A'(u^k)+\bar\alpha I)^{-1}$ существуют и ограничены \cite{Bak1976}, следовательно, процесс $\eqref{equ_rmn}$ определен корректно.
...
Сначала опишем экстремальные принципы, которые используются при их  построении для нелинейного монотонного оператора $A$. Используя разложение Тейлора в точке $u^k$ до первого порядка, получим линейное уравнение
\begin{equation*}
