...
\end{definition}
\begin{definition}
	Оператор $A$ называется равномерно монотонным, если $\exists \vartheta >0$ $\forall u,v\in H$ $$\langle A(u)-A(v), u-v\rangle \ge \vartheta\|u-v\|^2.$$
\end{definition}
\begin{definition}
	Оператор $A$ называется неотрицательно определенным, если $\forall u\in H$ выполняется неравенство $\langle A(u), u \rangle \ge 0$.
\end{definition}
\begin{definition}
